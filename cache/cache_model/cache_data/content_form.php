...
class content_form
{
	...
	function textarea($field, $value, $fieldinfo) {
		...
		$str .= ">{$value}</textarea>";
		if($maxlength) $str .= '还可以输入 <B><span id="'.$field.'_len">'.$maxlength.'</span></B> 个字符';
		return $str;
	}
	...
	function islink($field, $value, $fieldinfo) {
		...
		return '<input type="hidden" name="info[islink]" value="0"><input type="text" name="linkurl" id="linkurl" value="'.$url.'" size="'.$size.'" maxlength="255" '.$disabled.' class="input-text"> <input name="info[islink]" type="checkbox" id="islink" value="1" onclick="ruselinkurl();" '.$checked.'> <font color="red">转向链接地址</font>';
	}
	function template($field, $value, $fieldinfo) {
		$sitelist = getcache('sitelist','commons');
		$default_style = $sitelist[$this->siteid]['default_style'];
		return form::select_view($default_style,'content',$value,'name="info['.$field.']" id="'.$field.'"','show');
	}
