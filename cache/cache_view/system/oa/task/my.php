  ...
  <table class="table" width="100%" layoutH="56" rel="alllist">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td align="center">
          <?php if($r['status'] == 1) { ?><a href="?m=oa&c=task&a=accepted&id=<?php echo $r['id'];?>" target="ajaxTodo" title="确定要领取任务吗?">接受</a><?php } else { ?><font color="#cccccc">接受</font><?php } ?>
           | 
          <?php if($r['status'] > 1 && $r['status'] < 4) { ?><a href="?m=oa&c=task&a=progress&id=<?php echo $r['id'];?>" target="dialog" mask="true" maxable="false" rel="oa_task_progress" width="800" height="300">进度</a><?php } else { ?><font color="#cccccc">进度</font><?php } ?>
           | 
          <?php if($r['status'] > 1 && $r['status'] < 4) { ?><a href="?m=oa&c=task&a=complete&id=<?php echo $r['id'];?>" target="dialog" mask="true" maxable="false" rel="oa_task_complete" width="800" height="500">完成</a><?php } else { ?><font color="#cccccc">完成</font><?php } ?>
        </td>
      </tr>
      <?php } ?>
    </tbody>
