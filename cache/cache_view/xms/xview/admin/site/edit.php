...
<form action="<?php echo $this->_context->url('site::edit#'.$siteid.'@admin'); ?>" method="post" class="pageForm required-validate" onsubmit="return validateCallback(this, divAjaxDone);">
...
<fieldset>
  ...
  <tr>
    ...
    <td>
      ...
      <?php if(is_array($view_list)) { ?>
      <?php if(is_array($view_list)) foreach($view_list AS $key => $val) { ?>
      <option value="<?php echo $val;?>"<?php if($data['view'] == $val) { ?> selected<?php } ?>><?php echo $val;?></option>
      <?php } ?>
      <?php } ?>
      </select>
    </td>
