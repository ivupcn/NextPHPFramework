...
class iconv
{
	...
	public static function utf8_to_gbk($utfstr) {
		...
		$okstr = '';
		if(empty($UC2GBTABLE)) {
			$filename = Next_PATH.'data'.DIRECTORY_SEPARATOR.'encoding'.DIRECTORY_SEPARATOR.'gb-unicode.table';
	...
	public static function big5_to_gbk($Text) {
		static $BIG5_DATA;
		if(empty($BIG5_DATA)) {
			$filename = Next_PATH.'data'.DIRECTORY_SEPARATOR.'encoding'.DIRECTORY_SEPARATOR.'big5-gb.table';
			$fp = fopen($filename, 'rb');
	...
	public static function gbk_to_big5($Text) {
		static $GB_DATA;
		if(empty($GB_DATA)) {
			$filename = Next_PATH.'data'.DIRECTORY_SEPARATOR.'encoding'.DIRECTORY_SEPARATOR.'gb-big5.table';
			$fp = fopen($filename, 'rb');
	...
	public static function gbk_to_pinyin($txt) {
		...
		$pyarr = array();
		$py = array();
		$filename = Next_PATH.'data'.DIRECTORY_SEPARATOR.'encoding'.DIRECTORY_SEPARATOR.'gb-pinyin.table';
		$fp = fopen($filename,'r');
		while(!feof($fp)) {
