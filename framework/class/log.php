...
class NLOG
{
     ...
     * @var string
     */
    const HEADER_NAME = 'X-NextPHPChromeDebug-Data';
     ...
     * @param string label
     * @param mixed value
     * @param string severity NLOG::LOG || NLOG::WARN || NLOG::ERROR
     * @return void
     */
    ...
    protected static function _log(array $args)
    {
        ...
        $logger->_addRow($label, $value, $type);
    }
     ...
     * @return void
     */
    protected function _addRow($label, $log, $type)
    {
        $row = array($label, $log, $type);
    ...
    {
        // 确定日志写入目录
        $dir = Next::config('log', 'log_writer_dir', APP_PATH.'cache'.DIRECTORY_SEPARATOR.'log'.DIRECTORY_SEPARATOR);
        if (!is_dir($dir))
        {
            mkdir($dir, 0777, true);
        }
        ...
        if(is_file($this->_filename) && floor(Next::config('log', 'log_file_siz', '1024000')) <= filesize($this->_filename) )
        {
                rename($this->_filename,dirname($this->_filename).DIRECTORY_SEPARATOR.time().'-'.basename($this->_filename));
        }
        $string = '';
        do
        {
