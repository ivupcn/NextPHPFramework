<div class="pageContent">
  <div class="panelBar">
    <ul class="toolBar">
      ...
      <li><a class="icon" href="{url 'field::priview@content','modelid/'.$modelid}" target="dialog" mask="true" maxable="false" rel="content_file_priview" width="800" height="260"><span>预览模型</span></a></li>
    </ul>
  </div>
  ...
  <table class="table" width="100%" layoutH="92">
    ...
    <tbody>
      ...
      {loop $data $r}
      <tr>
        <td><input name="listorders[<?php echo $r['fieldid'];?>]" type="text" value="{$r['listorder']}" class="input-text-c" style="width:30px" /></td>
        <td>{$r['field']}</td>
        <td>{$r['name']}</td>
        <td>{$r['formtype']}</td>
        <td align="center">{php echo $r['issystem'] ? '<font color="red">√</font>' : '<font color="blue">×</font>'}</td>
