...
class extend_controller_workflow extends admin_class_controller
{
	...
	{
		$data = extend_model_workflow::model()->select(array('siteid'=>$this->get_siteid()));
		$this->_cache();
	...
	public function action_add()
	{
		...
		{
			$admin_data = array();
			$result = user_model_user::model()->select(array('siteid'=>$this->get_siteid()));
	...
	{
		$datas = array();
		$workflow_datas =  extend_model_workflow::model()->select(array('siteid'=>$this->get_siteid()),'*',1000);
		foreach($workflow_datas as $_k=>$_v)
		{
			$datas[$_v['workflowid']] = $_v;
		}
		setcache('workflow_'.$this->get_siteid(),$datas,'extend');
		return true;
	}
