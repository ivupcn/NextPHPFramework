...
class oa_controller_task extends admin_class_controller
{
	public function action_all()
	{
		...
		$data = oa_model_task::model()->listinfo(array('siteid'=>$this->get_siteid()), 'id DESC', $page, 20,'','','',1);
		$pages = oa_model_task::model()->pages;
		$userop = new user_class_userop();
		$category = getcache('category_oa_'.$this->get_siteid(),'admin');
	...
	{
		$user = user_model_user::model()->select(array('siteid'=>$this->get_siteid()),'userid,realname');
	...
	public function action_publish()
	{
		...
		{
			$user = user_model_user::model()->select(array('siteid'=>$this->get_siteid()));
	...
	private function looptask()
	{
		...
		$sql = "loopstatus = 0 AND looptask != 'once' AND siteid = ".$this->get_siteid();
