...
class user_controller_group extends admin_class_controller
{
	function action_init()
	{
		...
		$user_group_list = user_model_group::model()->listinfo(array('siteid'=>$this->get_siteid()), 'sort ASC', $page, 20);
	...
	function action_edit()
	{
		...
		{					
			$groupid = isset($_GET['groupid']) ? $_GET['groupid'] : $this->_app->showmessage('非法参数！', HTTP_REFERER);
	...
	private function _checkname($name = NULL)
	{
		...
		if (user_model_group::model()->get_one(array('name'=>$name,'siteid'=>$this->get_siteid()),'groupid'))
		{
			return false;
		}
		return true;
	...
	private function _updatecache() {
		$grouplist = user_model_group::model()->listinfo(array('siteid'=>$this->get_siteid()), '', 1, 1000, 'groupid');
		setcache('grouplist_'.$this->get_siteid(), $grouplist,'user');
	}
}
?>